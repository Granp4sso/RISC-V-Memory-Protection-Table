...
int read_test_cases(const char *input_file, test_case_t **test_cases) {
    ...
    *test_cases = (test_case_t *)malloc(test_count * sizeof(test_case_t));
    ...
    while (fgets(buffer, sizeof(buffer), file)) {
        char *tokens[9]; 
        int num_tokens = split_line(buffer, tokens, 9);
        ...
        (*test_cases)[i].mptl2_entry = strtoull(tokens[7], NULL, 16);
        (*test_cases)[i].mptl1_entry = strtoull(tokens[8], NULL, 16);

        // printf("Test Case %d: mptl2_entry = 0x%016llX\n", i, (unsigned long long)(*test_cases)[i].mptl2_entry);
        // printf("Test Case %d: mptl1_entry = 0x%016llX\n", i, (unsigned long long)(*test_cases)[i].mptl1_entry);
        // printf("Test Case %d: mmpt_reg = 0x%016llX\n", i, (unsigned long long)(*test_cases)[i].mmpt_reg);
        i++;
    }
...
void vmem_insert_entry(vmem_t* Memory, uint64_t mptl2_entry, uint64_t mptl1_entry) {
    /*
    for (int i = 0; i < 8; i++) {  
        uint8_t byte = (mptl2_entry >> (i * 8)) & 0xFF;
        Memory->mem[i].cell = byte;
    }
    printf("Memory contents:\n");
    for (int i = 0; i < 8; i++) {
        printf("mem[%d] = 0x%02X\n", i, Memory->mem[i].cell);
    }
    printf("mptl after calling vmem_insert_entry: 0x%016llX\n", (unsigned long long)mptl2_entry);
    */
   for (int i = 0; i < 8; i++) {
        Memory->mem[i].cell = (mptl2_entry >> (i * 8)) & 0xFF;
        Memory->mem[i + 8].cell = (mptl1_entry >> (i * 8)) & 0xFF;
    }
}
...
void run_simulation_and_save_results(const char *input_file, const char *output_file, uut_t* uut) {
    ...
    // Output CSV header
    fprintf(file, "mmpt_reg,spa_i,access_type_i,flush_i,allow_o,format_error_o,access_page_fault_o,mptl2_entry,mptl1_entry\n");
    ...
    for (int i = 0; i < SIM_CYCLES; i++) {
        ...
        if (waiting_for_result) {
            if (uut->mod->ptw_valid_o) {
                ...
                // Save results to CSV
               fprintf(file, "0x%016lx,0x%016lx,%d,%d,%d,%d,%d,0x%016lx,0x%016lx\n",
        ...
        // If NOT waiting for results, load a new test case
            ...
            // printf("Before calling vmem_insert_entry, mptl2_entry = 0x%016llX\n", (unsigned long long)mptl2_entry);
            uint64_t mptl1_entry = test_cases[test_index].mptl1_entry;
            // printf("Before calling vmem_insert_entry, mptl1_entry = 0x%016llX\n", (unsigned long long)mptl1_entry);


            // Insert the entry into the memory at the correct location, for example:
            vmem_insert_entry(&uut->vRAM, mptl2_entry, mptl1_entry);
            ...
            if (test_cases[test_index].flush) {
                ...
                fprintf(file, "0x%lx,0x%lx,%d,%d,%d,%d,%d,0x%lx,0x%lx\n",
                        test_cases[test_index].mmpt_reg, test_cases[test_index].spa_i,
...
int main(int argc, char **argv) {
    ...
    run_simulation_and_save_results("test_cases.csv", "output.csv", &uut);
