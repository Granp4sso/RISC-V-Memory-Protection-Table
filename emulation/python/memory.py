...
class Memory:
    def __init__(self, size, grant_delay=(1, 1), valid_delay=(1, 1)):
        """
    ...
    def cycle(self, clock_cycle, verbose=False):
        ...
        for addr, entry in enumerate(self.memory):
            delay = 0
            # Register request
            if self.req and self.addr == addr and not entry["requested"]:
            ...
            if entry["requested"] and not entry["granted"]:
                if (not self.req or self.addr != addr) and clock_cycle < entry["grant_cycle"]:
                    entry["requested"] = False
                    entry["grant_cycle"] = 0

            # Grant processing
            if clock_cycle == entry["grant_cycle"]:
                if self.req and self.addr == addr:
                    entry["granted"] = True
                    self.gnt = True
        ...
        self._prev_addr = self.addr

        if verbose:
            print(
                f"[{clock_cycle:03}] REQ={int(self.req)} ADDR={self.addr:03} | "
