...
# Data structure: data[locality_label][float_value] = list of (x_axis, min, avg, max)
...
for x_axis_value in sorted(os.listdir(RESULTS_DIR), key=lambda x: float(x)):
    ...
    for filename in os.listdir(folder_path):
        ...
        try:
            ...
            locality_label = parts[0]+parts[1]
        ...
        try:
            with open(file_path, "r") as f:
                ...
                data[locality_label][float_value].append(
...
for ax, (locality_label, series_dict) in zip(axs, data.items()):
    ...
    # Format subplot
    if "LOW" in locality_label:
        ax.set_title("Low Locality")
    elif "MEDIUM" in locality_label:
        ax.set_title("Medium Locality")
    else:
        ax.set_title("High Locality")
    
    ax.set_xticks(range(len(all_x_labels)))
    ax.set_xticklabels(all_x_labels)
    ax.set_xlabel("Forwarding Buffers Depth")
